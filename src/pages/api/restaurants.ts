...
import { getTopRestaurants } from "@/lib/services/attractions.service";
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Execute Effect - get top 10 scored restaurants
    const result = await Effect.runPromise(
      getTopRestaurants(lat, lng, apiKey, 10).pipe(
        Effect.catchAll((error) => {
          ...
          if (error._tag === "NoAttractionsFoundError") {
            return Effect.fail<ErrorResult>({
              ...
              message: `No restaurants found near (${error.location.lat}, ${error.location.lng})`,
            });
          }
