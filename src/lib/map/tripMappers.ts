 ...
 *
 * Purpose: Convert between map ViewModels and database types
 * - PlannedPlaceViewModel <-> PlaceDAO
 * - PlannedPOIViewModel <-> AttractionDAO
 * - TripPlaceDTO -> PlaceDAO
 */

import type { PlannedPlaceViewModel, PlannedPOIViewModel, PhotoViewModel } from "@/lib/map/types";
 ...
 * (Both use the same structure in map)
 */
...
/**
 * Convert TripPlaceDTO (from API) to PlaceDAO (for map state)
