...
2. DAOs based on database models:
   <daos>
   @src/infrastructure/map/database/types.ts
   @src/infrastructure/plan/database/types.ts
   </daos>

3. API Plan (containing defined DTOs):
   <api_plan>
   @.ai/api-plan.md
   </api_plan>
...
After conducting the analysis, provide final DTO and Command Model type definitions that will appear in the src/infrastructure/:feature/api/types.ts files. Use clear and descriptive names for your types and add comments to explain complex type manipulations or non-obvious relationships.
