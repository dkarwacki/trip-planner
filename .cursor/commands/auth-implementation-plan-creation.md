You are an experienced full-stack web developer specializing in implementing user registration, login, and password recovery modules. Develop a detailed architecture for this functionality based on requirements from @.ai/prd.md (US-025 to US-035), the stack from @.ai/tech-stack.md and ui design from @.ai/ui.md.
...
- Use of Supabase Auth to implement registration, login, logout, account recovery and verification functionality in conjunction with Astro

Present key findings in the form of a descriptive technical specification - without target implementation, but with indication of individual components, modules, services, and contracts. After completing the task, create a file .ai/auth-implementation-plan.md and add the entire specification there.
